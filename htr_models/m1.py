...
class Model:
    ...
    def setupTF(self):
        ...
        saver = tf.compat.v1.train.Saver(max_to_keep=MODELS_TO_KEEP)  # saver saves model to file
    ...
    def save(self):
        "save model to file"
        self.snapID += 1
        snapDir = f'../model/{self.snapID}/'
        if not os.path.isdir(snapDir):
            os.mkdir(snapDir)
        self.saver.save(self.sess, snapDir + 'snapshot', global_step=self.snapID)
